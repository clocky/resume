...
- **Styles**: `src/styles/style.scss` is compiled by Vite/Sass and injected as a hashed `<link>` by Astro. The `@import url(...)` for Noto Emoji is preserved at the top of the SCSS.
...
- Emoji section headings use the "Noto Emoji" web font loaded with a `text=` subset in `src/styles/style.scss`. When adding a new emoji heading, add that emoji to the `text=` list in the `@import url(...)` at the top of `src/styles/style.scss`, or it won't render in the intended font.
